...
if (isset($_GET['id'])) {
    ...
    try {
        ...
        $sql = "SELECT yonetici_adi, eposta, profil_resmi FROM Yonetici WHERE yonetici_id = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$id]);
        $yonetici = $stmt->fetch();

        if ($yonetici) {
            $yonetici_adi = $yonetici['yonetici_adi'];
            $eposta = $yonetici['eposta'];
            $profil_resmi = $yonetici['profil_resmi'];
        } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $eposta = $_POST['eposta'];
    $profil_resmi = $_POST['profil_resmi_guncel'];
    ...
    try {
        ...
        echo 'Veritabanı güncelleme hatası: ' . $e->getMessage();
        exit();
    }
}
?>
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <form action="yonetici_duzenle.php?id=<?= htmlspecialchars($id) ?>" method="POST" enctype="multipart/form-data">
                                            <div class="mb-3">
                                                <label for="yonetici_adi" class="form-label">Ad</label>
                                                <input type="text" class="form-control" id="yonetici_adi" name="yonetici_adi" value="<?= htmlspecialchars($yonetici_adi) ?>" required>
                                            </div>
                                            <div class="mb-3">
                                                <label for="eposta" class="form-label">Eposta</label>
                                                <input type="email" class="form-control" id="eposta" name="eposta" value="<?= htmlspecialchars($eposta) ?>" required>
                                            </div>
                                            <div class="mb-3">
                                                ...
                                                <input type="hidden" name="profil_resmi_guncel" value="<?= htmlspecialchars($profil_resmi) ?>">
                                                <?php if ($profil_resmi): ?>
                                                    <img src="<?= htmlspecialchars($profil_resmi) ?>" alt="Mevcut Profil Resmi" width="100">
                                                <?php endif; ?>
                                            </div>
